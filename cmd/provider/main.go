...
import (
	...
	apisCluster "github.com/rropen/provider-cscdm/apis/cluster"
	apisNamespaced "github.com/rropen/provider-cscdm/apis/namespaced"
	"github.com/rropen/provider-cscdm/config"
	"github.com/rropen/provider-cscdm/internal/clients"
	controllerCluster "github.com/rropen/provider-cscdm/internal/controller/cluster"
	controllerNamespaced "github.com/rropen/provider-cscdm/internal/controller/namespaced"
	"github.com/rropen/provider-cscdm/internal/features"
	"github.com/rropen/provider-cscdm/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for CscDM").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-cscdm"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-cscdm",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped CscDM APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced CscDM APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped CscDM controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced CscDM controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped CscDM controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced CscDM controllers")
	}
